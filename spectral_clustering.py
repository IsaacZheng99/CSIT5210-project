...
distances = np.linalg.norm(points_pca[:, np.newaxis] - points_pca, axis=-1)

# Compute the dc-dist similarity matrix
dc = np.median(distances)
similarity = np.exp(-distances**2 / (2 * dc**2))

# Perform spectral clustering
n_clusters = len(np.unique(labels))  # Number of clusters based on unique labels
spectral = SpectralClustering(n_clusters=n_clusters, affinity='precomputed')
cluster_labels = spectral.fit_predict(similarity)
