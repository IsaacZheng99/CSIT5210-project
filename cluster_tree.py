...
def copy_tree(root, min_points, pruned_parent=None):
    # The orig_node stores the infomation of original child nodes of a node
    if len(root) >= min_points:
        ...
        if root.left_tree is not None:
            pruned_root.set_left_tree(copy_tree(root=root.left_tree, min_points=min_points, pruned_parent=pruned_root))
        if root.right_tree is not None:
            pruned_root.set_right_tree(copy_tree(root=root.right_tree, min_points=min_points, pruned_parent=pruned_root))
...
def finalize_clusters(clusters):
    ...
    """
    # Thus, any 𝑘-center solution on the pruned tree corresponds to a 𝑞-coverage
    # solution on the original dataset. To show optimality, consider that Algorithm Tree-k-center
    # finds the optimal 𝑘-center solution on the pruned tree. Since solutions
    # on the pruned tree have equal cost to those on the original tree,
    # Algorithm Tree-k-Center-with-q-coverage returns the optimal 𝑘-center with 𝑞-coverage solution.
    epsilons = np.array([c.peak.dist for c in clusters])
    # MaxEpsilon(𝐶) gives the cost of the 𝑘-center solution
